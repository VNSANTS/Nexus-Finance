...
## Correção aplicada (rodada `tsc --noEmit` real, feita por você)

Você rodou `tsc --noEmit` de verdade e pegou um erro que minha checagem só
com esbuild (sintaxe, não tipos) não seria capaz de pegar:

```
seguranca.ts(69,23): error TS2322: Type 'Uint8Array<ArrayBufferLike>' is
not assignable to type 'BufferSource'. [...] Type 'SharedArrayBuffer' is
not assignable to type 'ArrayBuffer'.
```

Isso é uma mudança de tipagem do TypeScript 5.5+ (TypedArrays passaram a
ser genéricos sobre `ArrayBufferLike`) — a assinatura `function
hexParaBytes(hex: string): Uint8Array` virava `Uint8Array<ArrayBufferLike>`
por padrão, e `crypto.subtle.deriveBits` só aceita `BufferSource`
(que exige `ArrayBuffer`, não `ArrayBufferLike`, porque
`ArrayBufferLike` também cobre `SharedArrayBuffer`).

**Corrigido** em `seguranca.ts`: `hexParaBytes` agora declara o retorno
como `BufferSource` (com um cast interno seguro, já que o `Uint8Array` que
ela mesma cria nunca é `SharedArrayBuffer`), e `paraHex` aceita
`ArrayBufferLike` em vez de só `ArrayBuffer`. Nenhuma outra mudança de
comportamento — é só a assinatura de tipo.

Isso é exatamente o tipo de erro que só aparece com `tsc` real (não com
checagem de sintaxe via esbuild, que não faz inferência de tipos) — por
isso o aviso da seção abaixo continua valendo pra qualquer pendência nova.
